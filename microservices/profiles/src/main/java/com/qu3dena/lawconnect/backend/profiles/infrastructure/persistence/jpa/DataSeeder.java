...
public class DataSeeder {
    ...
    CommandLineRunner initProfilesDatabase(
        ...
        return args -> {
            ...
            boolean hasSpecialties = specialtyRepository.count() > 0;
            boolean hasLawyers = lawyerRepository.count() > 0;
            boolean hasClients = clientRepository.count() > 0;
            
            logger.info("Starting Profiles database seeding...");
            logger.info("Current state - Specialties: {}, Lawyers: {}, Clients: {}", 
                    specialtyRepository.count(), lawyerRepository.count(), clientRepository.count());
            ...
            // Create Lawyer Specialties (only if they don't exist)
            if (!hasSpecialties) {
                ...
                logger.info("✓ Lawyer specialties already exist ({} total)", specialtyRepository.count());
            }

            // Create Lawyer Profiles using real user IDs matched by username (only if they don't exist)
            if (!hasLawyers) {
                ...
                logger.info("✓ Lawyer profiles already exist ({} total)", lawyerRepository.count());
            }

            // Always create profiles for any LAWYER users in IAM that don't have a profile yet
            int createdLawyers = 0;
            for (Map<String, Object> user : users) {
                ...
                if ("ROLE_LAWYER".equals(role)) {
                    ...
                    if (lawyerRepository.findByUserId(userId).isEmpty()) {
                        // Extract first and last name from username
                        String[] nameParts = username.split("\\.");
                        String firstname = nameParts.length > 0 ? capitalize(nameParts[0]) : username;
                        String lastname = nameParts.length > 1 ? capitalize(nameParts[1]) : "Usuario";
                        
                        LawyerAggregate lawyer = LawyerAggregate.create(
                                userId,
                                new FullName(firstname, lastname),
                                new ContactInfo("+51 999999999", "Dirección no especificada"),
                                new Dni(generateDni()),
                                new Description("Perfil creado automáticamente para " + username),
                                null
                        );
                        lawyerRepository.save(lawyer);
                        createdLawyers++;
                        logger.info("✓ Created lawyer profile for user: {}", username);
                    }
                }
            }
            if (createdLawyers > 0) {
                logger.info("✓ Created {} additional lawyer profiles", createdLawyers);
            } else {
                logger.info("✓ All LAWYER users already have profiles");
            }

            // Create Client Profiles using real user IDs matched by username (only if they don't exist)
            if (!hasClients) {
                ...
                logger.info("✓ Client profiles already exist ({} total)", clientRepository.count());
            }

            // Always create profiles for any CLIENT users in IAM that don't have a profile yet
            ...
            for (Map<String, Object> user : users) {
                ...
                if ("ROLE_CLIENT".equals(role)) {
                    ...
                    if (clientRepository.findByUserId(userId).isEmpty()) {
                        ...
                        ClientAggregate client = ClientAggregate.create(
                                userId,
                                new FullName(firstname, lastname),
                                new ContactInfo("+51 999999999", "Dirección no especificada"),
                                new Dni(generateDni())
                        ...
                        createdClients++;
                        logger.info("✓ Created client profile for user: {}", username);
                    }
                }
            }
            if (createdClients > 0) {
                logger.info("✓ Created {} additional client profiles", createdClients);
            } else {
                logger.info("✓ All CLIENT users already have profiles");
            }
    ...
    /**
     * Generates a random DNI for auto-created profiles.
     */
    private String generateDni() {
        Random random = new Random();
        int dniNumber = 10000000 + random.nextInt(90000000);
        char letter = (char) ('A' + random.nextInt(26));
        return String.valueOf(dniNumber) + letter;
    }
}
